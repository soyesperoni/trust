...
export default function ClientesListadoPage() {
  ...
  const [statusFilter, setStatusFilter] = useState<"" | ClientRow["status"]>("");
  ...
  const uniqueContacts = useMemo(
    () => Array.from(new Set(clients.map((client) => client.contactName))).sort((a, b) => a.localeCompare(b)),
    [clients],
  );
  ...
  const filteredClients = useMemo(() => {
    ...
    return clients.filter((client) => {
      const matchesQuery =
        !query ||
        [client.name, client.contactName, client.contactEmail, client.status, String(client.id)].some((value) =>
          value.toLowerCase().includes(query),
        );
      const matchesStatus = !statusFilter || client.status === statusFilter;
      const matchesContact = !contactFilter || client.contactName === contactFilter;

      return matchesQuery && matchesStatus && matchesContact;
    });
  }, [clients, contactFilter, searchTerm, statusFilter]);
              ...
              <div className="p-4 border-b border-slate-100 dark:border-slate-800 space-y-3">
                ...
                <div className="grid grid-cols-1 gap-3 md:grid-cols-2">
                  <select
                    ...
                    value={statusFilter}
                    onChange={(event) => setStatusFilter(event.target.value as "" | ClientRow["status"])}
                  >
