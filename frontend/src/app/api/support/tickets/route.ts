...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    if (contentType.includes("multipart/form-data")) {
      ...
      response = await fetch(`${backendBaseUrl}/api/support/tickets/`, {
        method: "POST",
        headers: {
...
export async function DELETE(request: NextRequest) {
  ...
  try {
    const url = id
      ? `${backendBaseUrl}/api/support/tickets/${id}/`
      : `${backendBaseUrl}/api/support/tickets/`;
    ...
    return NextResponse.json({ error: "Error de conexión al eliminar ticket." }, { status: 502 });
  }
}
