...
class VisitMobileFlowTests(TestCase):
    ...
    @patch("core.views._send_email_async")
    @patch("core.views.VisitMedia.objects.create")
    def test_complete_visit_accepts_multipart_patch_payload(self, create_media_mock, send_email_async_mock, _build_pdf_mock):
        ...
        args = send_email_async_mock.call_args.args
        self.assertEqual(args[1][0], self.visit.id)
...
class IncidentPermissionsTests(TestCase):
    ...
    @patch("core.views._send_email_async")
    def test_branch_admin_can_create_incident(self, send_email_async_mock):
        response = self.client.post(
            "/api/incidents/",
            data=json.dumps(
                {
        ...
        send_email_async_mock.assert_called_once()
        args = send_email_async_mock.call_args.args
        created_incident_id = response.json()["id"]
        self.assertEqual(args[1][0], created_incident_id)
...
class AuditApiTests(TestCase):
    ...
    @patch("core.views._send_email_async")
    def test_complete_audit_mobile_flow(self, send_email_async_mock):
        form = AuditForm.objects.create(name="Checklist", schema={"questions": []})
        self.area.audit_form_template = form
        self.area.save(update_fields=["audit_form_template"])
        ...
        send_email_async_mock.assert_called_once()
        args = send_email_async_mock.call_args.args
        self.assertEqual(args[1][0], audit.id)
